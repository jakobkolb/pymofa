...
class experiment_handling(object):
    ...
    def resave(self, eva, name, no_output=False):
        ...
        Every parameter combination is sent off for evaluation once. If it does not
        come back correctly, it will be missing in the dataframe of results.
...
def even_time_series_spacing(dfi, n, t0=None, t_n=None):
    ...
    """
    if t0 is None or t0 < dfi.index.values[0]:
        t0 = dfi.index.values[0]
    if t_n is None or t_n > dfi.index.values[-1]:
